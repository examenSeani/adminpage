...
export class EtapaServicesService {
...
iniciar(etapaSeleccionada:string): void {
  //carga lista de alumnos de la segunda etapa en un arreglo "alumnoss"
  //alert(etapaSeleccionada);

  this.alumnosCollection = this.db.collection(etapaSeleccionada);
 // this.alumnosCollection = this.db.collection("alumnos3");

  //this.alumnosCollection = this.db.collection('e1-2022');
  this.alumnos = this.alumnosCollection.snapshotChanges().pipe(take(1),map(actions => {
      return actions.map(a => {
          const data = a.payload.doc.data() as segundoAlumno;//segundoAlumno es un modelo de datos
